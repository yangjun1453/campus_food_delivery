<template>
  <div class="order-container">
    ...
    <div class="order-list">
      ...
      <div v-if="unfinishedOrders.length === 0" class="order-empty">
        暂无未完成订单
      </div>
    </div>
  </div>
...
const fetchUnfinishedOrders = async () => {
  try {
    ...
    // 筛选未完成订单（状态不为 completed）
    unfinishedOrders.value = allOrders.filter(
...
onMounted(fetchUnfinishedOrders);
...
const completeOrder = async (orderId) => {
  try {
    ...
    alert("订单已标记为已完成");
    await fetchUnfinishedOrders(); // 刷新未完成订单列表
  } catch (error) {
